...
AS
	BEGIN
		-- Si no fue facturada previamente
		IF (@codigo_reserva NOT IN (SELECT codigo_reserva FROM GITAR_HEROES.Factura))
		BEGIN	
			IF (@codigo_reserva IN (SELECT codigo_reserva FROM GITAR_HEROES.Estadia) 
...
CREATE Procedure GITAR_HEROES.generarListado (@fecha_actual smalldatetime, @anio int, @trimestre smallint, @codigo_listado int)
AS
	BEGIN
		IF (@anio <= YEAR(@fecha_actual) AND @trimestre IN (1, 2, 3, 4))
		BEGIN	
